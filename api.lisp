...
(define-api courier/mail/trend (campaign &optional amount skip) (:access (perm courier user))
  (let ((campaign (check-accessible (ensure-campaign campaign) :target 'mail))
        (labels ())
        (data ()))
    (loop for mail in (dm:get 'mail (db:query (:= 'campaign (dm:id campaign)))
                              :sort '((time :DESC)) :amount (int* amount 10) :skip (int* skip 0))
